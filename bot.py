...
async def автодополнение_имплантов(interaction: discord.Interaction, current: str):
    ...
    все = []
    seen = set()
    for категория, предметы in шаблоны.items():
        for название, данные in предметы.items():
            if название.startswith("_"):
                for подназвание in данные:
                    if подназвание not in seen:
                        все.append(подназвание)
                        seen.add(подназвание)
            else:
                if название not in seen:
                    все.append(название)
                    seen.add(название)
    if not current:
        return [app_commands.Choice(name=n, value=n) for n in все[:25]]
...
async def персонаж(interaction: discord.Interaction, действие: str, имя_персонажа: str = None, игрок: discord.Member = None):
    ...
    if действие == "показать":
        ...
        embed.add_field(name="`[DER]` Производные", value=(
            f"❤️ Здоровье: **{p.get('здоровье_текущее', здоровье)}** / {здоровье}\n🤖 Человечность: **{человечность}**\n🎯 Инициатива: **{реакция}**\n💰 Эдди: **{p['эдди']}** €$"), inline=False)
        # Надетое снаряжение
        if p.get('надетое_снаряжение'):
            embed.add_field(name="`[GEAR]` Надето", value='\n'.join([f"👕 {item}" for item in p['надетое_снаряжение']]), inline=True)
        else:
            embed.add_field(name="`[GEAR]` Надето", value="Пусто", inline=True)

        # Броня (только надетая)
        ...
        # Оружие в руках и быстром доступе
        сп = p.get('оружие', [])
        руки = p.get('руки', {"правая": None, "левая": None})
        бд = p.get('быстрый_доступ', [])
        оруж_стр = []
        for r in ["правая", "левая"]:
            idx = руки[r]
            if idx is not None and idx < len(сп):
                оруж_стр.append(f"✋ {r}: **{сп[idx]['название'].title()}**")
        if бд:
            for idx in бд:
                if idx < len(сп):
                    оруж_стр.append(f"⚡ БД: **{сп[idx]['название'].title()}**")
        if оруж_стр:
            embed.add_field(name="`[WPN]` Оружие", value='\n'.join(оружие_стр), inline=True)

        # Импланты: стилевые и борг
        уст_имп = p.get('установлено_импланты', {})
        имп_стр = []
        for имп in уст_имп.get('стилевые', []):
            имп_стр.append(f"✨ {имп['название'].title()} — {имп['шаблон']['эффект']}")
        for имп in уст_имп.get('активные', []):
            if имп['шаблон'].get('борг_слота'):
                имп_стр.append(f"⚡ {имп['название'].title()} — {имп['шаблон']['эффект']}")
        if имп_стр:
            embed.add_field(name="`[CYB]` Импланты", value='\n'.join(имп_стр), inline=True)

        # Статус ранений
        ранение = статус_ранения(p)
        if ранение == "смертельно_ранен":
            embed.add_field(name="`[WND]` Ранение", value="💀 **СМЕРТЕЛЬНО РАНЕН**\n*Штраф -4 ко всем броскам и СКО*", inline=False)
        elif ранение == "ранен":
            embed.add_field(name="`[WND]` Ранение", value="🩹 **Ранен**\n*Штраф -2 ко всем броскам*", inline=False)

        if человечность < 0:
            embed.add_field(name="`[PSY]` Статус", value="**НЕКОНТРОЛИРУЕМЫЙ КИБЕРПСИХ**\n*Управление невозможно.*", inline=False)
        elif эмпатия == 0:
            embed.add_field(name="`[PSY]` Статус", value="Киберпсихоз\n*Штраф -2 к броскам харизмы*", inline=False)
        elif эмпатия == 1:
            embed.add_field(name="`[PSY]` Статус", value="-# *Диссоциативное расстройство*\n-# *Штраф -1 к броскам харизмы*", inline=False)
        ...
        персонажи[цель_id] = {
            ...
            "навыки": {}, "удача_текущая": 1, "здоровье_текущее": 5, "удача_последняя_трата": None, "человечность": 10, "базовая_эмпатия": 1,
...
def статус_ранения(p):
    здоровье = p.get('здоровье_текущее', 5)
    макс_здоровье = 5 * ((p['статы'].get('телосложение', 1) + p['статы'].get('сила воли', 1)) // 2)
    половина = (макс_здоровье + 1) // 2  # округление вверх
    if здоровье <= 0:
        return "смертельно_ранен"
    elif здоровье < половина:
        return "ранен"
    return None
...
async def бросок(interaction: discord.Interaction, навык: str, удача: int = 0, мод: int = 0):
    ...
    if ранение == "ранен":
        штраф_ранения = -2
    elif ранение == "смертельно_ранен":
        штраф_ранения = -4

    общий = стат_бонус + ур + бонус_удачи + мод + псих + штраф_ранения
    ...
    await interaction.response.send_message(
        ...
        (f"\n║ Ранение: {штраф_ранения}" if штраф_ранения else "") +
        f"{взрыв_текст}\n║ Итого: **{результат}**\n╚═══ {тип} ═══╝"
    )
...
@app_commands.describe(действие="Что сделать", предмет="Номер (для продажи) или название (для покупки)")
@app_commands.choices(действие=[
    app_commands.Choice(name="инвентарь", value="инвентарь"),
    app_commands.Choice(name="установленные", value="установленные"),
    app_commands.Choice(name="купить", value="купить"),
    app_commands.Choice(name="продать", value="продать"),
])
async def кибер(interaction: discord.Interaction, действие: str, предмет: str = None):
    ...
    if действие == "инвентарь":
        if not p['киберимпланты']:
            await interaction.response.send_message("🔌 Инвентарь пуст.")
            return
        строки = ["**🔌 Киберимпланты в инвентаре:**"]
        for i, item in enumerate(p['киберимпланты'], 1):
            строки.append(f"{i}. ▸ {item['название'].title()} — {item['шаблон']['эффект']} (слот: {item['шаблон']['слот']})")
        await interaction.response.send_message('\n'.join(строки))

    elif действие == "установленные":
        уст_имп = p.get('установлено_импланты', {})
        все_уст = []
        ...
        if not все_уст:
            await interaction.response.send_message("⚡ Установленных имплантов нет.")
            return
        строки = ["**⚡ Установленные импланты:**"]
        for i, (слот, имп) in enumerate(все_уст, 1):
            if слот == "активные":
                строки.append(f"{i}. ⚡ **{имп['название'].title()}** — {имп['шаблон']['эффект']}")
            else:
                строки.append(f"{i}. ⚡ {слот}: **{имп['название'].title()}** — {имп['шаблон']['эффект']}")
        await interaction.response.send_message('\n'.join(строки))
        ...
        for категория, предметы in шаблоны.items():
            for название, данные in предметы.items():
                if not название.startswith("_"):
                    if название == предмет:
                        ш = данные
                        break
                else:
                    if предмет in данные:
                        ш = данные[предмет]
                        break
            if ш: break
                ...
                for название, данные in предметы.items():
                    if not название.startswith("_"):
                        ...
                        for подназвание in данные:
                            все.append(подназвание)
            все = list(set(все))
            await interaction.response.send_message(f"`[ERR]` Доступные: {', '.join(все)}", ephemeral=True)
            return
...
async def установить_кибер(interaction: discord.Interaction, действие: str, номер: int):
    ...
    if действие == "установить":
        ...
        if имп['шаблон'].get('открывает'):
            ...
            if 'активные' not in p['установлено_импланты']:
                p['установлено_импланты']['активные'] = []
            p['установлено_импланты']['активные'].append(имп)
            save_characters(персонажи)
...
async def атака(interaction: discord.Interaction, рука: str = "правая", удача: int = 0, мод: int = 0):
    ...
    # Определяем оружие или кулаки
    if экип is not None and экип < len(сп):
        оруж = сп[экип]
        название = оруж['название'].title()
        ш = оруж['шаблон']
        навык = ш['навык']
        урон_формула = ш['урон']
    else:
    ...
    общий = стат_бонус + ур + бонус_удачи + мод + штраф_ранения
    бросок = random.randint(1, 10)
    результат = бросок + общий
    ...
    # Бросок урона
    ч = урон_формула.split('d')
    кубы = int(ч[0])
    грани = int(ч[1])
    броски_урона = [random.randint(1, грани) for _ in range(кубы)]
    урон = sum(броски_урона)

    # Крит урона: если две d6 выпали на 6
    крит_урон = ""
    if кубы >= 2 and грани == 6:
        шестёрки = броски_урона.count(6)
        if шестёрки >= 2:
            урон += 5
            крит_урон = f"\n║ 💥 КРИТ УРОНА! +5 (две 6 на d6)"

    await interaction.response.send_message(
        f"`[ATK]` {interaction.user.mention} ({рука}): **{название}**\n"
        f"╔═══ АТАКА ═══╗\n║ D10: **{бросок}**\n║ Навык: +{ур}\n║ {SHORT_STATS[SKILLS[навык]['стат']]}: +{стат_бонус}" +
        (f"\n║ Удача: +{бонус_удачи}" if бонус_удачи else "") +
        (f"\n║ Мод: {мод:+}" if мод else "") +
        (f"\n║ Ранение: {штраф_ранения}" if штраф_ранения else "") +
        f"{взрыв_текст}\n║ Итого: **{результат}**\n╠═══ УРОН ═══╣\n║ {урон_формула}: {', '.join(map(str, броски_урона))}\n║ Урон: **{урон}**{крит_урон}\n╚═══ {тип} ═══╝"
    )

# ==========================================
# ПОЛУЧИТЬ УРОН
# ==========================================
@bot.tree.command(name="урон", description="Получить урон (учитывает броню)")
@app_commands.describe(количество="Сколько урона", часть_тела="Куда попали")
@app_commands.choices(часть_тела=[
    app_commands.Choice(name="тело", value="тело"),
    app_commands.Choice(name="голова", value="голова"),
    app_commands.Choice(name="без брони", value="нет"),
])
async def урон(interaction: discord.Interaction, количество: int, часть_тела: str = "тело"):
    персонажи = load_characters()
    автор_id = str(interaction.user.id)
    if автор_id not in персонажи:
        await interaction.response.send_message("`[ERR]` Создай персонажа!", ephemeral=True)
    ...
    защита = 0
    if часть_тела != "нет":
        надета = p.get('надето_броня', {})
        предмет = надета.get(часть_тела)
        if предмет:
            защита = предмет['шаблон']['защита']
            название_брони = предмет['название'].title()
        else:
            название_брони = None
    else:
        название_брони = None
    ...
    # Итоговый урон
    итоговый_урон = max(0, количество - защита)

    # Текущее здоровье
    тело = p['статы'].get('телосложение', 1)
    воля = p['статы'].get('сила воли', 1)
    макс_здоровье = 5 * ((тело + воля) // 2)
    if 'здоровье_текущее' not in p:
        p['здоровье_текущее'] = макс_здоровье

    старое = p['здоровье_текущее']
    p['здоровье_текущее'] = max(0, старое - итоговый_урон)
    save_characters(персонажи)
    ...
    if часть_тела != "нет":
        if название_брони:
            броня_стр = f"\n🛡️ {часть_тела.title()}: **{название_брони}** (защита: {защита})"
        else:
            броня_стр = f"\n🛡️ {часть_тела.title()}: без брони"
    ...
    await interaction.response.send_message(
        f"`[DMG]` {interaction.user.mention} получает урон!\n"
        f"╔═══ УРОН ═══╗\n"
        f"║ Входящий: {количество}\n"
        f"║ Защита: -{защита}{броня_стр}\n"
        f"║ Итого: **{итоговый_урон}**\n"
        f"║ Здоровье: {старое} → **{p['здоровье_текущее']}** / {макс_здоровье}\n"
        f"╚═══ {статус_стр} ═══╝"
    )
...
# РЕСУРСЫ
# ==========================================
@bot.tree.command(name="ресурсы", description="[Гейммастер] Выдать или забрать ресурсы")
@app_commands.describe(игрок="Кому", действие="Выдать или забрать", тип="Что", количество="Сколько")
@app_commands.choices(действие=[
    app_commands.Choice(name="выдать", value="выдать"),
    app_commands.Choice(name="забрать", value="забрать"),
])
@app_commands.choices(тип=[
    app_commands.Choice(name="О.У.", value="оу"),
    app_commands.Choice(name="Эдди", value="эдди"),
    app_commands.Choice(name="Здоровье", value="здоровье"),
    app_commands.Choice(name="Удача", value="удача"),
    app_commands.Choice(name="Человечность", value="человечность"),
])
async def ресурсы(interaction: discord.Interaction, игрок: discord.Member, действие: str, тип: str, количество: int):
    if "Гейммастер" not in [р.name for р in interaction.user.roles]:
        await interaction.response.send_message("`[ERR]` Только Гейммастер!", ephemeral=True)
        return
    персонажи = load_characters()
    ...
    знак = "+" if действие == "выдать" else "-"
    изменение = количество if действие == "выдать" else -количество

    if тип == "оу":
        p['оу'] += изменение
        save_characters(персонажи)
        await interaction.response.send_message(f"`[ОУ]` {знак}{количество} для {игрок.mention}. Всего: **{p['оу']}**")

    elif тип == "эдди":
        p['эдди'] += изменение
        save_characters(персонажи)
        await interaction.response.send_message(f"`[€$]` {знак}{количество} для {игрок.mention}. Баланс: **{p['эдди']}**")

    elif тип == "здоровье":
        тело = p['статы'].get('телосложение', 1)
        воля = p['статы'].get('сила воли', 1)
        макс_здоровье = 5 * ((тело + воля) // 2)
        # Здоровье храним как отдельное поле, если нет — создаём
        if 'здоровье_текущее' not in p:
            p['здоровье_текущее'] = макс_здоровье
        p['здоровье_текущее'] += изменение
        if p['здоровье_текущее'] < 0:
            p['здоровье_текущее'] = 0
        if p['здоровье_текущее'] > макс_здоровье:
            p['здоровье_текущее'] = макс_здоровье
        save_characters(персонажи)
        await interaction.response.send_message(f"`[❤️]` {знак}{количество} здоровья для {игрок.mention}. Текущее: **{p['здоровье_текущее']}** / {макс_здоровье}")

    elif тип == "удача":
        макс_удача = p['статы'].get('удача', 1)
        if 'удача_текущая' not in p:
            p['удача_текущая'] = макс_удача
        p['удача_текущая'] += изменение
        if p['удача_текущая'] < 0:
            p['удача_текущая'] = 0
        if p['удача_текущая'] > макс_удача:
            p['удача_текущая'] = макс_удача
        save_characters(персонажи)
        await interaction.response.send_message(f"`[🍀]` {знак}{количество} удачи для {игрок.mention}. Текущая: **{p['удача_текущая']}** / {макс_удача}")

    elif тип == "человечность":
        if 'человечность' not in p:
            p['человечность'] = p['статы'].get('эмпатия', 1) * 10
        старая = p['человечность']
        p['человечность'] += изменение
        базовая = p.get('базовая_эмпатия', p['статы'].get('эмпатия', 1))
        if p['человечность'] < 0:
            p['человечность'] = 0
        if p['человечность'] > базовая * 10:
            p['человечность'] = базовая * 10
        p['статы']['эмпатия'] = p['человечность'] // 10
        save_characters(персонажи)
        псих = ""
        if p['человечность'] < 0: псих = "\n💀💀💀 НЕКОНТРОЛИРУЕМЫЙ КИБЕРПСИХ! 💀💀💀"
        elif p['человечность'] // 10 == 0: псих = "\n⚠️ КИБЕРПСИХОЗ!"
        elif p['человечность'] // 10 == 1: псих = "\n*Диссоциативное расстройство*"
        await interaction.response.send_message(f"`[🤖]` Человечность {игрок.mention}: {старая} → **{p['человечность']}**{псих}")
